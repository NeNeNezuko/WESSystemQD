...
public class WavePickTaskReportController : ControllerBase
{
    private readonly WarehouseDbContext _context;
    private readonly ILogger<WavePickTaskReportController> _logger;

    public WavePickTaskReportController(WarehouseDbContext context, ILogger<WavePickTaskReportController> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    /// <summary>
    /// 查询波次拣货任务单报表（明细表以TF_JHRW表身为主，统计表以MF_JHRW表头为主）
    /// </summary>
    [HttpGet("search")]
    public async Task<ActionResult<ApiResult<List<WavePickTaskReportDto>>>> Search(
        ...
        [FromQuery] string? tabType = "detail")
    {
        try
        {
            if (tabType == "summary")
            {
                ...
                if (!string.IsNullOrWhiteSpace(bcNo))
                    headerQuery = headerQuery.Where(m => m.TZ_NO != null && m.TZ_NO.Contains(bcNo));

                headerQuery = headerQuery.OrderBy(m => m.JR_NO);

                var headerList = await headerQuery.ToListAsync();
                var list = headerList.Select(m => new WavePickTaskReportDto
                {
                    ItemNo = 0,
                    JrNo = m.JR_NO ?? "",
                    JrDd = m.JR_DD,
                    ...
                    TypeId = m.TYPE_ID ?? "",
                    ClsId = m.CLS_ID ?? "N",
                    PriorityWcs = m.PRIORITY_WCS.HasValue ? m.PRIORITY_WCS.Value.ToString() : "",
                    WorkStation = m.WORK_STATION ?? "",
                    Usr = m.USR ?? "",
                    SysDate = m.SYS_DATE
                }).ToList();

                return Ok(new ApiResult<List<WavePickTaskReportDto>>
                {
                    Success = true,
                    Data = list,
                    Total = list.Count
            ...
            else
            {
                ...
                if (!string.IsNullOrWhiteSpace(bcNo))
                    query = query.Where(x => x.T.TZ_NO != null && x.T.TZ_NO.Contains(bcNo));

                query = query.OrderBy(x => x.T.JR_NO).ThenBy(x => x.T.ITM);

                var rawList = await query.ToListAsync();
                var list = rawList.Select(x => new WavePickTaskReportDto
                {
                    ItemNo = x.T.ITM,
                    JrNo = x.T.JR_NO ?? "",
        ...
        {
            _logger.LogError(ex, "查询波次拣货任务单报表时发生错误");
            return StatusCode(500, new ApiResult<List<WavePickTaskReportDto>>
            {
                Success = false,
...
/// <summary>波次拣货任务单报表DTO</summary>
public class WavePickTaskReportDto
{
    ...
    public string PriorityWcs { get; set; } = "";
