using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;
using WmsPlus.Api.Data;
using WmsPlus.Api.Models;

namespace WmsPlus.Api.Controllers;

[Authorize]
[ApiController]
[Route("api/[controller]")]
public class OutboundNoticeReportController : ControllerBase
{
    private readonly WarehouseDbContext _context;
    private readonly ILogger<OutboundNoticeReportController> _logger;

    public OutboundNoticeReportController(WarehouseDbContext context, ILogger<OutboundNoticeReportController> logger)
    {
        _context = context;
        _logger = logger;
    }

    /// <summary>
    /// 查询出库通知单报表（支持明细表/统计表两种视图）
    /// </summary>
    [HttpGet("search")]
    public async Task<ActionResult<ApiResult<List<object>>>> Search(
        [FromQuery] DateTime? dateFrom,
        [FromQuery] DateTime? dateTo,
        [FromQuery] string? noticeNumber,
        [FromQuery] string? warehouseCode,
        [FromQuery] string? vendorCode,
        [FromQuery] string? closeStatus,
        [FromQuery] string? viewType = "detail")  // detail=明细表, summary=统计表
    {
        try
        {
            if (viewType == "summary")
            {
                return await SearchSummary(dateFrom, dateTo, noticeNumber, warehouseCode, vendorCode, closeStatus);
            }
            else
            {
                return await SearchDetail(dateFrom, dateTo, noticeNumber, warehouseCode, vendorCode, closeStatus);
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "查询出库通知单报表时发生错误");
            return StatusCode(500, new ApiResult<List<object>>
            {
                Success = false,
                Message = $"服务器内部错误: {ex.Message}"
            });
        }
    }

    private async Task<ActionResult<ApiResult<List<object>>>> SearchDetail(
        DateTime? dateFrom, DateTime? dateTo,
        string? noticeNumber, string? warehouseCode, string? vendorCode, string? closeStatus)
    {
        var query = from t in _context.TfCktzs
                    join m in _context.MfCktzs on t.TZ_NO equals m.TZ_NO into mj
                    from m in mj.DefaultIfEmpty()
                    select new { T = t, M = m };

        if (dateFrom.HasValue)
            query = query.Where(x => x.M != null && x.M.TZ_DD >= dateFrom.Value);
        if (dateTo.HasValue)
            query = query.Where(x => x.M != null && x.M.TZ_DD <= dateTo.Value.AddDays(1).AddSeconds(-1));
        if (!string.IsNullOrWhiteSpace(noticeNumber))
            query = query.Where(x => x.T.TZ_NO.Contains(noticeNumber));
        if (!string.IsNullOrWhiteSpace(warehouseCode))
            query = query.Where(x => x.M != null && (x.M.WH != null && x.M.WH.Contains(warehouseCode)));
        if (!string.IsNullOrWhiteSpace(vendorCode))
            query = query.Where(x => x.M != null && (x.M.CUS_NO != null && x.M.CUS_NO.Contains(vendorCode)));
        if (!string.IsNullOrWhiteSpace(closeStatus) && closeStatus != "全部")
        {
            var isClosed = closeStatus == "已结案" ? "Y" : "N";
            query = query.Where(x => x.M != null && x.M.CLS_ID == isClosed);
        }

        query = query.OrderBy(x => x.T.TZ_NO).ThenBy(x => x.T.ITM);

        var rawList = await query.ToListAsync();

        var list = rawList.Select(x => new OutboundNoticeReportDetailDto
        {
            ItemNo = x.T.ITM,
            TZ_NO = x.T.TZ_NO ?? "",
            TZ_DD = x.M?.TZ_DD,
            WH = x.M?.WH ?? "",
            CUS_NO = x.M?.CUS_NO ?? "",
            CUS_NAME = x.M?.CUS_NAME ?? "",
            PRD_NO = x.T.PRD_NO ?? "",
            PRD_NAME = x.T.PRD_NAME ?? "",
            PRD_MARK = x.T.PRD_MARK ?? "",
            BAT_NO = x.T.BAT_NO ?? "",
            VALID_DD = x.T.VALID_DD,
            UNIT = x.T.UNIT ?? "",
            QTY = x.T.QTY ?? 0,
            REM = x.T.REM ?? ""
        }).Cast<object>().ToList();

        return Ok(new ApiResult<List<object>> { Success = true, Data = list, Total = list.Count });
    }

    private async Task<ActionResult<ApiResult<List<object>>>> SearchSummary(
        DateTime? dateFrom, DateTime? dateTo,
        string? noticeNumber, string? warehouseCode, string? vendorCode, string? closeStatus)
    {
        var query = _context.MfCktzs.AsQueryable();

        if (dateFrom.HasValue)
            query = query.Where(m => m.TZ_DD >= dateFrom.Value);
        if (dateTo.HasValue)
            query = query.Where(m => m.TZ_DD <= dateTo.Value.AddDays(1).AddSeconds(-1));
        if (!string.IsNullOrWhiteSpace(noticeNumber))
            query = query.Where(m => m.TZ_NO.Contains(noticeNumber));
        if (!string.IsNullOrWhiteSpace(warehouseCode))
            query = query.Where(m => m.WH != null && m.WH.Contains(warehouseCode));
        if (!string.IsNullOrWhiteSpace(vendorCode))
            query = query.Where(m => m.CUS_NO != null && m.CUS_NO.Contains(vendorCode));
        if (!string.IsNullOrWhiteSpace(closeStatus) && closeStatus != "全部")
        {
            var isClosed = closeStatus == "已结案" ? "Y" : "N";
            query = query.Where(m => m.CLS_ID == isClosed);
        }

        query = query.OrderBy(m => m.TZ_NO);

        var rawList = await query.ToListAsync();

        var list = rawList.Select(m => new OutboundNoticeReportSummaryDto
        {
            TZ_NO = m.TZ_NO,
            TZ_DD = m.TZ_DD,
            WH = m.WH ?? "",
            DEP = m.DEP ?? "",
            CUS_NO = m.CUS_NO ?? "",
            CUS_NAME = m.CUS_NAME ?? "",
            SAL_NO = m.SAL_NO ?? "",
            BIL_TYPE = m.BIL_TYPE ?? "",
            CLS_ID = m.CLS_ID ?? "",
            USR = m.USR ?? "",
            SYS_DATE = m.SYS_DATE
        }).Cast<object>().ToList();

        return Ok(new ApiResult<List<object>> { Success = true, Data = list, Total = list.Count });
    }
}

// ====== DTO 定义 ======

public class OutboundNoticeReportDetailDto
{
    public int ItemNo { get; set; }
    public string TZ_NO { get; set; } = "";
    public DateTime? TZ_DD { get; set; }
    public string WH { get; set; } = "";
    public string CUS_NO { get; set; } = "";
    public string CUS_NAME { get; set; } = "";
    public string PRD_NO { get; set; } = "";
    public string PRD_NAME { get; set; } = "";
    public string PRD_MARK { get; set; } = "";
    public string BAT_NO { get; set; } = "";
    public DateTime? VALID_DD { get; set; }
    public string UNIT { get; set; } = "";
    public decimal QTY { get; set; }
    public string REM { get; set; } = "";
}

public class OutboundNoticeReportSummaryDto
{
    public string TZ_NO { get; set; } = "";
    public DateTime? TZ_DD { get; set; }
    public string WH { get; set; } = "";
    public string DEP { get; set; } = "";
    public string CUS_NO { get; set; } = "";
    public string CUS_NAME { get; set; } = "";
    public string SAL_NO { get; set; } = "";
    public string BIL_TYPE { get; set; } = "";
    public string CLS_ID { get; set; } = "";
    public string USR { get; set; } = "";
    public DateTime? SYS_DATE { get; set; }
}
