...
public class OutboundOrderReportController : ControllerBase
{
    ...
    private readonly ILogger<OutboundOrderReportController> _logger;

    public OutboundOrderReportController(WarehouseDbContext context, ILogger<OutboundOrderReportController> logger)
    {
        _context = context;
        _logger = logger;
    }
        ...
        [FromQuery] string? viewType = "detail")
    {
        ...
        {
            if (viewType == "summary")
                return await SearchSummary(dateFrom, dateTo, orderNumber, warehouseCode, customerCode, closeStatus);
            else
                return await SearchDetail(dateFrom, dateTo, orderNumber, warehouseCode, customerCode, closeStatus);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "查询出库单报表时发生错误");
    ...
    private async Task<ActionResult<ApiResult<List<object>>>> SearchDetail(
        ...
        string? orderNumber, string? warehouseCode, string? customerCode, string? closeStatus)
    {
        ...
        if (!string.IsNullOrWhiteSpace(orderNumber))
            query = query.Where(x => x.T.CK_ID.Contains(orderNumber));
        ...
        var list = rawList.Select(x => new OutboundOrderReportDetailDto
        {
            ItemNo = x.T.ITM,
            CK_NO = x.T.CK_ID ?? "",
            CK_DD = x.M?.CK_DD,
            WH = x.M?.WH ?? "",
            CUS_NO = x.M?.CUS_NO ?? "",
            CUS_NAME = x.M?.CUS_NAME ?? "",
            PRD_NO = x.T.PRD_NO ?? "",
            PRD_NAME = x.T.PRD_NAME ?? "",
            PRD_MARK = x.T.PRD_MARK ?? "",
            BAT_NO = x.T.BAT_NO ?? "",
            CHUW = x.T.WH ?? "",
            UNIT = x.T.UNIT ?? "",
            QTY = x.T.QTY ?? 0,
    ...
    private async Task<ActionResult<ApiResult<List<object>>>> SearchSummary(
        ...
        string? orderNumber, string? warehouseCode, string? customerCode, string? closeStatus)
    {
        var query = _context.MfCks.AsQueryable();

        if (dateFrom.HasValue)
            query = query.Where(m => m.CK_DD >= dateFrom.Value);
        if (dateTo.HasValue)
            query = query.Where(m => m.CK_DD <= dateTo.Value.AddDays(1).AddSeconds(-1));
        if (!string.IsNullOrWhiteSpace(orderNumber))
            query = query.Where(m => m.CK_ID.Contains(orderNumber));
        if (!string.IsNullOrWhiteSpace(warehouseCode))
            query = query.Where(m => m.WH != null && m.WH.Contains(warehouseCode));
        if (!string.IsNullOrWhiteSpace(customerCode))
            query = query.Where(m => m.CUS_NO != null && m.CUS_NO.Contains(customerCode));
        if (!string.IsNullOrWhiteSpace(closeStatus) && closeStatus != "全部")
        {
            var isClosed = closeStatus == "已结案" ? "Y" : "N";
        ...
        query = query.OrderBy(m => m.CK_ID);

        var rawList = await query.ToListAsync();

        var list = rawList.Select(m => new OutboundOrderReportSummaryDto
        {
            CK_NO = m.CK_ID,
            CK_DD = m.CK_DD,
            WH = m.WH ?? "",
...
public class OutboundOrderReportDetailDto
{
...
public class OutboundOrderReportSummaryDto
{
    public string CK_NO { get; set; } = "";
    public DateTime? CK_DD { get; set; }
    public string WH { get; set; } = "";
